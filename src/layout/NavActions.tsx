        ...
        <AnimatePresence onExitComplete={handleExitComplete}>
          {panelVisible ? (
            ...
            >
              ...
              <form className="mt-3 flex flex-col gap-3" role="search" onSubmit={handleSearch}>
                <Input
                  type="search"
                  value={query}
                  onChange={(event) => setQuery(event.target.value)}
                  placeholder={intl.formatMessage({ id: 'nav.searchPlaceholder' })}
...
export const NavActions = () => {
  ...
  return (
    <>
      ...
      <Link to={getLocalizedPath('/cart', locale)} aria-label={intl.formatMessage({ id: 'nav.cart' })}>
        <motion.span
          className={iconClassName}
          whileHover={{ scale: 1.08 }}
          whileTap={{ scale: 0.92 }}
