...
const OrderConfirmationIllustration = () => {
  const intl = useIntl()
  ...
  return (
    <motion.div
      ...
      aria-label={intl.formatMessage({
        id: 'checkout.confirmationIllustration',
        defaultMessage: 'A glowing parcel with a confirmation check',
      })}
    ...
    >
      ...
      >
        <svg viewBox="0 0 200 200" className="size-full">
          ...
          <path d="M94 70c0-8 3-14 6-14s6 6 6 14" className="fill-none stroke-tertiary" strokeWidth="2.5" />

          <circle cx="148" cy="58" r="22" className="fill-secondary" />
          <path
            d="M139 58.5 145.5 65l13-16"
...
export const OrderConfirmationView = () => {
  ...
  return (
    <section className="flex min-h-[calc(100svh-12rem)] items-center justify-center px-4 py-10">
      <motion.div
      ...
      >
        ...
        <p className="mt-6 text-xs font-medium tracking-[0.22em] text-secondary uppercase">
          {intl.formatMessage({ id: 'checkout.confirmationKicker', defaultMessage: 'On its way' })}
        </p>
        <h1 className="mt-2 max-w-xs text-xl font-medium text-balance md:text-2xl">
          {intl.formatMessage({ id: 'checkout.successTitle', defaultMessage: 'Order confirmed' })}
        </h1>
        <p className="mt-2 max-w-sm text-sm leading-relaxed text-tertiary">
          {intl.formatMessage({
            id: 'checkout.successBody',
            defaultMessage: 'Pay in cash when your order is delivered in Egypt.',
          })}
        </p>

        <p className="mt-5 rounded-full bg-primary/40 px-4 py-1.5 text-sm font-medium text-foreground">
          {intl.formatMessage({ id: 'checkout.orderNumber', defaultMessage: 'Order number' })}{' '}
        ...
        <motion.div className="mt-2 w-full max-w-56" whileHover={{ scale: 1.03 }} whileTap={{ scale: 0.97 }}>
          <Link
            to={ordersPath}
            viewTransition={true}
            className={cn(
          ...
          >
            ...
            {intl.formatMessage({ id: 'checkout.backToOrders', defaultMessage: 'View orders' })}
          </Link>
        </motion.div>
      </motion.div>
    </section>
